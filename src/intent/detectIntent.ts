 ...
 * - own_token_sentiment: User asks about bot's feeling/view on own token ($twimsalot)
 */
...
- own_token_sentiment: User asks how the bot feels about its own token ($twimsalot / twimsalot / our token / dein token)
 ...
 * Covers: $twimsalot, twimsalot, twimsalot, our token, dein token, your token, own token
 */
export function detectOwnTokenSentiment(text: string): boolean {
  ...
  const OWN_TOKEN_PATTERNS = [
    ...
    /\byour\s*token\b/i,
    /\bown\s*token\b/i,
    /\b(feeling|think|feel|meinung|sicht|view).*(twimsalot|our token|dein token|your token)\b/i,
    /\b(twimsalot|our token|dein token).*(feeling|think|feel|meinung|sicht|view|bullish|bearish)\b/i,
