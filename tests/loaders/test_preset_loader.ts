...
describe("presetLoader", () => {
  describe("resolvePresetKey", () => {
    it("returns aliased key for legacy horny_ prefixed keys", () => {
      expect(resolvePresetKey("horny_roast_card")).toBe("gorky_roast_card");
      expect(resolvePresetKey("horny_cyberpunk")).toBe("gorky_cyberpunk");
    });

    it("returns original key if no alias", () => {
      expect(resolvePresetKey("custom_preset")).toBe("custom_preset");
      expect(resolvePresetKey("gorky_roast_card")).toBe("gorky_roast_card");
    });

    it("handles all legacy HORNY -> GORKY aliases", () => {
      const legacyKeys = [
        "horny_roast_card",
        "horny_cyberpunk",
        "horny_chart_ghost",
      ...
      for (const key of legacyKeys) {
        ...
        expect(resolved.startsWith("gorky_")).toBe(true);
  ...
  describe("loadPreset", () => {
    it("loads a valid preset YAML", () => {
      ...
      try {
        const presetPath = resolve("./prompts/presets/images/gorky_roast_card.yaml");
  ...
  describe("getPresetByKey with aliases", () => {
    it("finds preset via alias", () => {
      ...
      const mockPresets = new Map([
        ["gorky_roast_card", { preset_key: "gorky_roast_card", style_prompt: "test", sourcePath: "" }],
      ]);

      const preset = getPresetByKey(mockPresets, "horny_roast_card");

      expect(preset).toBeDefined();
      expect(preset?.preset_key).toBe("gorky_roast_card");
    });
  });
});
